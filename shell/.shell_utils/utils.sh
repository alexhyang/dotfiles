...
# SHELL
# =========================================================
reload_shell() { # reload shell (TODO: split into shell-based modules)
  case "$(ps -p $$ -o comm=)" in
    "zsh")
      echo "reload zsh..."
      omz reload
...
rm_hist() { # remove command history that contains given pattern
  sed -i "/$1/d" ~/.zsh_history
}

rg_hist() { # show commands in zsh history
  rg $1 ~/.zsh_history
}
